    ...
    while let Ok(message) = msg_rx.recv_async().await {
        match message.instruction {
            Instruction::AreaSubscribe => {
                handle_area_subscribe(message, &peer_map, &mut world_map).await?
            }
            Instruction::AreaUnsubscribe => {
                handle_area_unsubscribe(message, &peer_map, &mut world_map).await?
            }
            Instruction::GlobalMessage => handle_global_message(message, &peer_map).await?,
            Instruction::LocalMessage => {
                handle_local_message(message, &peer_map, &mut world_map).await?
            }
