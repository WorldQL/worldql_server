...
pub async fn start_outgoing_zeromq_thread(mut msg_rx: UnboundedReceiver<MessageAndClientUUID>, ctx: tmq::Context) -> Result<()> {
    ...
    loop {
        ...
        if mc.message.instruction == Instruction::Handshake && !mc.message.parameter.is_none() {
            let endpoint = "tcp://".to_owned() + &mc.message.parameter.ok_or_else(|| DecodeError::MissingRequiredField("parameter on Handshake".into()))?;
            println!("{}", endpoint);
            ...
            outgoing_socket.unwrap().send(vec![outgoing_message.serialize()]).await;
